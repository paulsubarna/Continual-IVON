...
class IVON_wprior(torch.optim.Optimizer):
    """IVON extended for continual learning via a Bayesian prior mechanism."""

    hessian_approx_methods = ('bonnet', 'gradsq', 'exact')
    ...
    def __init__(
        ...
        defaults = dict(
            lr=lr, mc_samples=mc_samples, beta1=beta1, beta2=beta2,
    ...
    def _restore_param_average(self, train, param_avg, noise):
        if train:
        ...
        for group in self.param_groups:
            for i, p in enumerate(group["params"]):
                ...
                if train:
                    ...
                    group["avg_grad"][i] = _welford_mean(group["avg_grad"][i], grad_sample, count)
                    ...
                    group["avg_lhess"][i] = _welford_mean(group["avg_lhess"][i], lhess_sample, count)
                param_idx += 1
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group["params"]):
                ...
                p.data = self._new_param_averages(
                    p.data, prior_mean, group["hess"][i], group["momentum"][i],
                    lr, wd, clip_radius, debias, group["hess_init"], self.alpha, group["ess"],
                )

    @staticmethod
    def _get_nll_hess(method, hess, avg_lhess, ess=None, wd=None):
        if method == 'bonnet':
            return avg_lhess * hess
        elif method in ('gradsq', 'exact'):
            return avg_lhess
        else:
    ...
    def _new_hess(method, hess, avg_lhess, ess, beta2, wd):
        f = IVON_wprior._get_nll_hess(method, hess + wd, avg_lhess)
        if method == 'bonnet':
            f = f * ess
    ...
    def get_prior(self):
        ...
        for num_, group in enumerate(self.param_groups):
            ...
            for p, h, wd, prior_m, prec_p in zip(
                group["params"], group["hess"], group["weight_decay"], prior_means, prior_prec
            ):
                if p is None:
                    continue
    ...
    def set_for_new_task(self, ess, hess_init, model=None):
        """Snapshot current Hessian as prior and reset optimizer state for the next task."""
        prior_mean, prior_precision = self.get_prior()
        for group in self.param_groups:
            group["ess"] = ess
            group["hess_init"] = hess_init
        self.current_step = 0
        self._reset_samples()
        self._init_buffers()
